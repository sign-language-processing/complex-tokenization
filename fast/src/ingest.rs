...
//! boundary crossing takes the raw text list and does pretokenization, word
//! dedup, graph building, and Trainer construction entirely in Rust.
//! Pretokenization runs the same `tokenizers` crate the Python side wraps
//! (`Split(Regex(GPT_PATTERN), behavior="isolated")`), so splits are
//! identical by construction. The Python shim gates this on the default
//! pretokenizer, `utf8_clusters` units, and no registered script handlers;
//! anything else takes the per-document Python path.
...
// Mirrors GPT_PATTERN in complex_tokenization/graphs/words.py verbatim.
const GPT_PATTERN: &str = "[^\\r\\n\\p{L}\\p{N}]?[\\p{Lu}\\p{Lt}\\p{Lm}\\p{Lo}\\p{M}]*[\\p{Ll}\\p{Lm}\\p{Lo}\\p{M}]+(?i:'s|'t|'re|'ve|'m|'ll|'d)?|[^\\r\\n\\p{L}\\p{N}]?[\\p{Lu}\\p{Lt}\\p{Lm}\\p{Lo}\\p{M}]+[\\p{Ll}\\p{Lm}\\p{Lo}\\p{M}]*(?i:'s|'t|'re|'ve|'m|'ll|'d)?|\\p{N}{1,3}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n/]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+";

fn gpt_splitter() -> Split {
    Split::new(
        SplitPattern::Regex(GPT_PATTERN.into()),
        SplitDelimiterBehavior::Isolated,
        false,
    )
    .expect("GPT_PATTERN is a valid onig regex")
}

fn pretokenize<'a>(splitter: &Split, text: &'a str) -> Vec<&'a str> {
    let mut pts = PreTokenizedString::from(text);
    splitter
        .pre_tokenize(&mut pts)
        .expect("Split pre-tokenization is infallible for plain text");
    pts.get_splits(OffsetReferential::Original, OffsetType::Byte)
        .into_iter()
        .map(|(_, (start, end), _)| &text[start..end])
        .collect()
}
...
pub(crate) fn build_corpus_graph(texts: &[String], connected: bool) -> GraphV {
    let splitter = gpt_splitter();
    let doc_tokens: Vec<Vec<&str>> = texts
        .par_iter()
        .map(|t| pretokenize(&splitter, t))
        .collect();
